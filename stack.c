...
int main() {
    char oriAry[101];   // 先分配101 * sizeof(char) 個bytes。
    char stack[101];    // 先分配101 * sizeof(char) 個bytes。
    int i;
    ...
    int len = (UINT32) strlen(oriAry) - 1;  // 因為strlen回傳型態為「unsigned long」，所以要轉型。而這裡「選用unsigned int為因結果一定大於0」。另外，「-1是為了扣除之前的Enter字元」。
